...
class SettingsWindow(QDialog):
    ...
    def auto_detect_rcon(self):
        ...
        port_text = self.vars['RCon_Port'].text().strip()
        try:
            port = int(port_text)
        except ValueError:
            port = 27015

        password = self.vars['RCon_Password'].text()
        found = self.logic.discover_rcon_host(port=port, password=password)
        ...
        if found:
            ...
            custom_popup(
                ...
                "This has been written to the RCon Address field. "
